...
double distanceFromLimelightToGoalInches = (goalHeightInches - limelightLensHeightInches)/Math.tan(angleToGoalRadians);
  ...
  public void robotInit() {
    ...
    rightEncoder = new Encoder(8,9 , true,EncodingType.k4X);
  ...
  public void autonomousInit() {
    ...
    LeftEncoderTarget[1] = LeftEncoderTarget[0] - 12.5*3.14;
    //RightEncoderTarget[1] = RightEncoderTarget[0] - 4900;
    LeftEncoderTarget[2] = LeftEncoderTarget[1] + 50;
    LeftEncoderTarget[3] = LeftEncoderTarget[2] -12.5*3.14;
    LeftEncoderTarget[4] = LeftEncoderTarget[0] - 50;
    //RightEncoderTarget[2] = RightEncoderTarget[1] + 5800;
    step = 0;
    autoPID = new PIDController(.085, 0, 0);
  ...
  /*
  AUTOS:
  ThreeBallAuto = start with two preloaded, fire both, pick up a third and fire
  TwoBallAuto = start with two preloaded, fire both, back up 
  */
  public void ThreeBallAuto(){
    ...
    if(autonTimer.get()<3.5){
      ...
      if(tv.getDouble(0)==1){
        ...
        //}
          ...
          hoodTarget = arr[0];
          targetRPM = arr[1];
          if((hoodTarget-1)<=hoodReadingtoDegrees(hoodMotor.getSelectedSensorPosition()) && (hoodTarget+1)>=hoodReadingtoDegrees(hoodMotor.getSelectedSensorPosition()) && (Math.abs(horizontalOffset) < 2 && Math.abs(rpm-targetRPM) <= 100)){
            SmartDashboard.putBoolean("hood +rpm", true);
            feed.set(ControlMode.PercentOutput, .5);
            lowerHopper.set(ControlMode.PercentOutput, hopperSpeed);
            upperHopper.set(ControlMode.PercentOutput, hopperSpeed);
    ...
    if(step ==3){
      LeftMaster.set(ControlMode.PercentOutput,  .54 * autoPID.calculate(leftEncoder.getDistance()-LeftEncoderTarget[1], LeftEncoderTarget[2]-LeftEncoderTarget[1]));
      RightMaster.set(ControlMode.PercentOutput, -.6 * autoPID.calculate(leftEncoder.getDistance()-LeftEncoderTarget[1], LeftEncoderTarget[2]-LeftEncoderTarget[1]));
    }
    if( Math.abs(leftEncoder.getDistance()-LeftEncoderTarget[1])<=3 && step == 2){
      step++;
    }
    if(Math.abs(leftEncoder.getDistance()-LeftEncoderTarget[2])<=3 && step == 3){
      step++;
      h = autonTimer.get();
    }
    if(step==4){
    ...
    if(autonTimer.get()>h + 3 && step == 4){
      step++;
    }
    if(Math.abs(leftEncoder.getDistance()-LeftEncoderTarget[3])<=3 && step == 5){
      step++;
    }
    if(step == 5){
  ...
  public void TwoBallAuto(){
    ...
    if(step==2){
      //intakeMotor.set(ControlMode.PercentOutput, intakeSpeed);
      LeftMaster.set(ControlMode.PercentOutput,  .45 * autoPID.calculate(leftEncoder.getDistance()-LeftEncoderTarget[0], LeftEncoderTarget[4]-LeftEncoderTarget[0]));
      RightMaster.set(ControlMode.PercentOutput, -.5 * autoPID.calculate(leftEncoder.getDistance()-LeftEncoderTarget[0], LeftEncoderTarget[4]-LeftEncoderTarget[0]));
    }
    if(Math.abs(leftEncoder.getDistance()-LeftEncoderTarget[4])<=1 && step == 2){
      step++;
    }
  }
}
